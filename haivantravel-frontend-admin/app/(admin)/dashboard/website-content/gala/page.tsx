...
export default function WebsiteContentGalaPage() {
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      try {
        const res = await fetch(`${apiBaseUrl}/gala`, { cache: "no-store" });
        if (!res.ok) return;
        const data = await res.json();
        if (!cancelled && data?.content != null) setContent(String(data.content));
      } catch (error) {
        console.error("[Gala] fetch error", error);
      }
    };
  ...
  const handleSave = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`${apiBaseUrl}/gala`, {
        method: "POST",
      ...
      setMessage("Đã lưu nội dung Gala.");
    } catch (error) {
