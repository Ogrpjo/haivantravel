...
export default function WebsiteContentRecruitmentPage() {
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      try {
        const res = await fetch(`${apiBaseUrl}/recruitment`, {
          cache: "no-store",
        });
        if (!res.ok) return;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu nội dung tuyển dụng.");
    } catch (error) {
      setMessage(
      ...
      <section className="bg-[#121212] text-white flex-8 px-[20px] w-full py-[10px]">
        <div className="py-[10px]">
          <p className="text-xl font-semibold text-white/75">
            {">"} Website Content {">"} Recruitment
          </p>
        </div>
        <div className="rounded-lg bg-[#1a1a1a] p-6 h-screen flex flex-col gap-4 border border-white/10">
