...
export default function WebsiteContentTeamBuildingPage() {
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      try {
        const res = await fetch(`${apiBaseUrl}/teambuilding`, { cache: "no-store" });
        if (!res.ok) return;
        const data = await res.json();
        if (!cancelled && data?.content != null) setContent(String(data.content));
      } catch (error) {
        console.error("[Team building] fetch error", error);
      }
    };
  ...
  const handleSave = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`${apiBaseUrl}/teambuilding`, {
        method: "POST",
      ...
      setMessage("Đã lưu nội dung Team building.");
    } catch (error) {
